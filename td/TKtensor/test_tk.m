...
% ERR=norm(TK2tensor(T)-TK2tensor(T2));
...
% Err1 = norm(X1-X)/sqrt(prod(X.size));
Err2 = norm(calculate('minus', X2, X))/sqrt(prod(X.size));
% Err2 = norm(X2-X)/sqrt(prod(X.size));
...
% Err3 = norm(X3-X)/sqrt(prod(X.size));
Err4 = norm(calculate('minus', X4, X))/sqrt(prod(X.size));
% Err4 = norm(X4-X)/sqrt(prod(X.size));
