...
para.alg='ls';
B_ls=lr(X,Y,para);
% rmse_ls=root_mse(Y,X*B_ls);
rmse_ls=norm(Y-X*B_ls)^2;
disp('B_ls=');
disp(B_ls);
...
para.alg='gd';
B_gd=lr(X,Y,para);
% rmse_gd=root_mse(Y,X*B_gd);
rmse_gd=norm(Y-X*B_gd)^2;
disp('B_gd=');
disp(B_gd);
...
rmse_ls=0;
for m=1:M
    % rmse_ls=rmse_ls+para.w(m).*root_mse(Y{m},X{m}*B_ls);
    rmse_ls=rmse_ls+para.w(m).*norm(Y{m}-X{m}*B_ls)^2;
end
disp('B_ls=');
disp(B_ls);
disp(['RMSE=',num2str(rmse_ls)]);
...
rmse_gd=0;
for m=1:M
    % rmse_gd=rmse_gd+para.w(m).*root_mse(Y{m},X{m}*B_gd);
    rmse_gd=rmse_gd+para.w(m).*norm(Y{m}-X{m}*B_gd)^2;
end
disp('B_gd=');
disp(B_gd);
disp(['RMSE=',num2str(rmse_gd)]);
