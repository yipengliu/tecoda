...
for i = 1:num_train
    ...
    for r = 1:R
        % D(:,:,:,(i-1)*R+r) = outprod(T.factors{1}(:,r),outprod(T.factors{2}(:,r),T.factors{3}(:,r)));
        D(:,:,:,(i-1)*R+r) = outer_product(T.factors{1}(:,r),outer_product(T.factors{2}(:,r),T.factors{3}(:,r)));
    end
end
D = D./max(D(:));
...
for i = 1:itter
    X1 = double(mode_n_product(D,G',N+1)); % reconstructed tensor of the previous iteration
    fprintf('Iteration: %d\n',i)
    % Update A
    A = (mode_n_unfold(Xtrain,N+1)*(mode_n_unfold(D,N+1))'+Y+p*G)*pinv(mode_n_unfold(D,N+1)*(mode_n_unfold(D,N+1))'+p*eye(K,K));
    % Update G
    tmp = A-(1/p)*Y;
    G = Sthresh(tmp,round((1-sp)*K)); % hard-thresholding
    % Update the dictionary D
    D = D+double(mode_n_product(Xtrain,(pinv(A))',N+1));
    D = D./max(D(:));
    % Update Y
    Y = Y+p*(G-A);
    
    % Stopping criterion
    Xtn = double(mode_n_product(D,G',N+1)); % reconstructed tensor
...
for i = 1:itter
    ...
    fprintf('Iteration: %d\n',i)
    % Update A
    A = (mode_n_unfold(Xtest,N+1)*(mode_n_unfold(D,N+1))'+Y+p*G)*pinv(mode_n_unfold(D,N+1)*(mode_n_unfold(D,N+1))'+p*eye(K,K));
    % Update G
    tmp = A-(1/p)*Y;
    G = Sthresh(tmp,round((1-sp)*K)); % hard-thresholding
    % Update Y
...
for i = 1:num_test
    ...
    Mrec{i} = double(mode_n_product(D,lam_dec,N+1));
