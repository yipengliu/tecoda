...
while((change_A>tol_A))   
  ...
  X_new_whole = khatrirao_product(full(C_new),full(B_new))';
  X_new_whole = sparse(X_new_whole);
  ...
  %% Arora "Unbiased" 
   ...
   for iii = 1:m
       ...
       B_temp = A_arora_red(:,dict_ele) - (Kronecker_product((A_arora_red(:,iii)'*A_arora_red(:,dict_ele)),(A_arora_red(:,iii))));
   ...
   for iii = 1:m
       ...
       gr_arora_red(:,iii) =  mean((Y_m - squeeze(B(:,:,iii))*temp_x_i).*Kronecker_product(sign(XS_arora_red(iii,:)), ones(n,1)), 2);
   end
