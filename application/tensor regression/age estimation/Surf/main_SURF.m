function [cvLambda,cvTestErr,optW,resSD,optiLambda,SolutionPath,TestErrInterp,TestRSD,cvSigma] = main_SURF(Xten,Xt,Y,R,alpha,epsilon);
...
% R =30;                         % Rank of CP decomposition
MaxIter = 1000;                 % Length of solution path
early_stop = 1;
nstep_wait = 300;
% alpha = 0;                      % L2-norm regularized parameter
    ...
    [cvLambda{r,1}, cvTestErr{r,1},cvW,cvSigma] = cvfun(cvp, Xten, Xt, Y,epsilon,xi,MaxIter,alpha,early_stop,nstep_wait);
     ...
     if sum(W) ==0
         ...
         if abs(resSD(r)-lastSD) < threshold 
             ...
             optW(r,:) = W;
             lastSD = resSD(r);
%              if sum(sum(optW,1)~=0)>=opabsw
%                  fprintf('The number of selected features is too large \n');
%                  break;
%              end
         end
...
%% Compute the average L1 norm of W
    ...
    for i=1:5
        ...
        if ~isempty(idxx)
            ...
            wt = double(CP2tensor(CPtensor(wt)))* sigma{i,1}(max(idxx));
