...
t = 1;
XY = Xt' * Y;%% the correlation of x and y
diag_XX = sum(Xt.^2, 1);%%
...
wt = double(CP2tensor(CPtensor(wt))) * sigma(t); 
LastW = wt(:);
...
while 1
    ...
    wt = double(CP2tensor(CPtensor(wt))) * sigma(t+1);
    W = wt(:);
    if sum(abs(W)) < absW && t<10
 %% Update Z_n
        inds_n = DIM;
        inds_n{m_hat} = i_hat;
        Xten_m = Xten(inds_n{:});
        if N==2
            ...
            for i = 1:N
                if i~=m_hat
                    ...
                    tmp = mode_n_product(Xten_m,w(t+1,mode_n),mode_n);   
