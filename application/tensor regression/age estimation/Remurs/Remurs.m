...
size_tX  = size(tX);
dim      = size_tX(1:N);
X        = mode_n_unfold(tX, N+1);%Unfold(tX, size(tX), N+1); % Data in vector form. Matrix: M*d.
Xty      = X'*y;
numFea   = size(X,2);
numSam   = size(X,1);
...
for k = 1:max_iter
    ...
    for n = 1:N
        tV{n} = double(mode_n_fold( prox_nuclear_norm( mode_n_unfold(tW-tB{n}, n), alpha/rho/N ),  n, dim));
