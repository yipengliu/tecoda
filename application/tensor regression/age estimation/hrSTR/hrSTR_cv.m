function [lambda_optimal,R_optimal,cvfold]=hrSTR_cv(X,y,cv_fold,settings)
    ...
    for k=1:length(Rlist)
        ...
        for i = 1:cvp.NumTestSets
            ...
            for j = 1 :  size(ytest, 2)
                ...
                [U, d, train_err] = linTensorSVR(TrainData, Ytrain(:,l), lambda, R);     % tensor svr
            ...
            for m = 1 : 1:   size(ytest, 1)
                ...
                for s = 1 : size(ytest, 2)
                    ...
                    ten_U = ktensor(U);
                    ten_U = tensor(ten_U);
                    YPt(m, s) = innerprod(Xdata, ten_U)+d;
                end
            end
            %% save result
