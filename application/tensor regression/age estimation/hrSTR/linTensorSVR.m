function [U, d, err] = linTensorSVR(InputData, InputTarget, C, R, MaxIter, Tol, Uini)
% higher rank support tensor regression 
% Input:
%      InputDate : the Data Tensor, I1xI2x...xI_MxN, N is the number of
%                  training data, and M is the modes of a single tensor
%                  observation
%      InputTarget : the output variable : Nx1
%      C      :    the regularization paramter for SVR
%      R           : the rank
%      MaxIter     : the maximum number iterations
%      Tol         : the tolerance for convergence 
%      Uini        : the initialized factor matrices 
% Output
%      U : the factor matrices of regression weights
%      d : the bias
%      err : the trainign error
% Dependencies : tensor toolbox
%
% Written by Weiwei Guo
% Dependencies : tensor toolbox, libsvm
%
% Written by Weiwei Guo

if ~exist('lambda', 'var') || isempty(lambda)
    C = 0.05; % regularization parameter
end
if ~exist('R', 'var') || isempty(R)
    R = 3; % rank
end
if ~exist('MaxIter', 'var') || isempty(MaxIter)
    MaxIter = 30; % the maximum number of iteration
end
if ~exist('Tol', 'var') || isempty(Tol)
    Tol = 1e-6; % the Tolerance of convergence
end

Dims = size(InputData);
N = Dims(end); % the number of training data
ndim = length(Dims) - 1;
U = cell(1, ndim);
if ~exist('Uini', 'var') || isempty(Uini)
    % Initialization
    % randomized
    for i = 1 : ndim
        u = rand(Dims(i), R);
        u = -1+ 2*u;
        U{i} = u;
    end
end
U=CPtensor(U);

%% Iteration
Loop = 1;
iter = 0;
libsvm_options = sprintf('-s %d -t %d -c %f', 3, 0, C);



while(Loop)
    iter = iter + 1;
    Uold = U;
    
    for i = 1 : ndim
        
        Phi = zeros(N, prod(size(U.factors{i})));
        
        % project the data
        % computing matrix B
        B = ones(R, R);
        for j = [1:i-1 i+1:ndim]
            B = B.*((U.factors{j})'*U.factors{j});
        end
        % sqrt B and inverse sqrt B
        [sqrtB resnorm] = sqrtm(B);
        invsqrtB = real(inv(sqrtB));
        for n = 1 : N
            Xdata = eval(['InputData(' repmat(':,', 1, ndim) num2str(n) ')']);
            xi = mtkr_product(Xdata, U.factors, i);%mttkrp(Xdata, U, i);
            xi = xi*invsqrtB;
            Phi(n,:) = (xi(:))';            
        end
        
        % support vector regression
%         [ignorce dim] = size(Phi);
        model =  fitrsvm(Phi,InputTarget);
        ww = model.Beta;
        d = model.Bias;
%         if isempty(model.SVs)
%             libsvm_options = sprintf('-s %d -t %d -c %f', 4, 2, C);
%                model = svmtrain(InputTarget,Phi,libsvm_options);
%         end
        % compute the weights
%         model = svmtrain(InputTarget,Phi,libsvm_options);
%         ww = model.SVs'*model.sv_coef;
%         d = -model.rho;
        if isempty(ww)
            ww=zeros(Dims(i), R);
        end
        ww_tmp = reshape(ww, Dims(i), R); 
        ww_tmp = ww_tmp*invsqrtB;
        
        U.factors{i} = ww_tmp;     

    end
    
   
    ten_U_old = CP2tensor(Uold);
    ten_U = CP2tensor(U);
    
    norm_u = norm(ten_U);
    norm_u_gap =  norm(calculate('minus',ten_U,ten_U_old));%norm(ten_U - ten_U_old);
    tor = norm_u_gap/norm_u;
    
    % check the termination conditions
    fprintf('Iteration:%d, Tor:%f\n', iter, tor);
    if(iter>MaxIter || tor<Tol)
        Loop = 0;
    end
end

pred = zeros(N, 1);
for n = 1 : N
    ten_U = CP2tensor(U);
  %  ten_U = tensor(ten_U);    
    Xdata = eval(['InputData(' repmat(':,', 1, ndim) num2str(n) ')']);    
    pred(n) = inner_product(Xdata, ten_U)+d;
end
err = InputTarget - pred;
err = sqrt(mean((err).^2));