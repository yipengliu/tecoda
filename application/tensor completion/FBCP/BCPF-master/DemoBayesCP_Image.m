...
[model] = BCPF_IC(Y, 'obs', O, 'init', 'rand', 'maxRank', 100, 'maxiters', 100, ...
...
[model] = BCPF_MP(Y, 'obs', O, 'init', 'rand', 'maxRank', 100, 'maxiters', 100, ...
    'tol', 1e-4, 'dimRed', 1, 'verbose', 1, 'nd', nd);
...
SSIMlist
TimeCost
norm(X_FBCP(:)-X(:),2)/norm(X(:),2)
norm(X_FBCPS(:)-X(:),2)/norm(X(:),2)
close all
figure(1);
imshow(uint8(X_FBCP),'border','tight');
figure(2);
imshow(uint8(X_FBCPS),'border','tight');